...
LD_FLAGS += -T$(LINKER_SCRIPT) -Map $(BUILD_DIR)/$(OUTPUT_FILE).map
LD_FLAGS += -u call_user_start

.PHONY: build clean upload fresh

build: $(BUILD_DIR)/$(OUTPUT_FILE)-0x00000.bin dissasembly
	@echo done!

$(BUILD_DIR)/$(OUTPUT_FILE)-0x00000.bin: $(BUILD_DIR)/$(OUTPUT_FILE).elf	
...
dissasembly: $(BUILD_DIR)/$(OUTPUT_FILE).elf
	@echo dissasembling
...
$(BUILD_DIR)/$(OUTPUT_FILE).elf: $(OBJ) $(BUILD_DIR)/app_partition.o
	@echo linking
	@$(LD) $(LD_FLAGS) -o $@ $^
